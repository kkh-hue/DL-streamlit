...
class CNN(nn.Module):
    def __init__(self, conv1=32, conv2=64, hidden=128, dropout=0.3):
        ...
        self.features = nn.Sequential(nn.Conv2d(1, conv1, kernel_size=3, padding=1),                                                 # ← ...을 지우고 위 주석의 코드를 직접 타이핑
                    nn.ReLU(),
        ...
        self.classifier = nn.Sequential(nn.Flatten(),                        # (N, conv2, 7, 7) → (N, conv2*49)
    ...
    def forward(self, x):
        ...
        # raise ScaffoldIncomplete("TODO 1-C: CNN.forward의 데이터 흐름을 완성하세요.")
...
def load_model():
    ...
    # raise ScaffoldIncomplete("TODO 2: 체크포인트에서 모델 구조와 가중치를 복원하세요.")
...
def preprocess_image(image: Image.Image) -> tuple[torch.Tensor, np.ndarray]:
    ...
    # raise ScaffoldIncomplete("TODO 3: 28×28 배열을 CNN 입력 tensor로 변환하세요.")
    x = canvas.reshape(1, 1, 28, 28).astype(np.float32) / 255.0
...
def predict_probabilities(model: nn.Module, x: torch.Tensor) -> np.ndarray:
    ...
    # raise ScaffoldIncomplete("TODO 4: inference_mode·순전파·softmax를 연결하세요.")
